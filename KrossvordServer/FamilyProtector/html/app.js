let app = angular.module("app", ['stringUtil', 'ui.scroll']).controller("myCtrl", function($scope, $http, stringUtil,
    ...
    $scope.location = function(m) {
        ...
        $http.get('https://lookin24.com/sendCommand?imei='+$scope.selectedName.imei+'&sendLocation=1&curDay='+$scope.curDay.getTime()+'&off='+$scope.curDay.getTimezoneOffset()).then(function (d) {
    ...
    $scope.Web = function(m) {
        if(m != 0) $scope.curDay = new Date();
        $scope.loadingdiv = true;
        $http.get('https://lookin24.com/sendCommand?imei='+$scope.selectedName.imei+'&sendWebsites=1&curDay='+$scope.curDay.getTime()+'&off='+$scope.curDay.getTimezoneOffset()).then(function (d) {
            console.log(d.data.data);
            $scope.loadingdiv = false;
            if(d.data.data == undefined) {
                return;
    ...
    $scope.gundelik = function(m) {
        ...
        $http.get('https://lookin24.com/sendCommand?imei='+$scope.selectedName.imei+'&curDay='+$scope.curDay.getTime()+'&off='+$scope.curDay.getTimezoneOffset()).then(function (d) {
            $scope.loadingdiv = false;
            console.log(d.data.data);
    ...
    $scope.yact = function(m) {
        if(m != 0) $scope.curDay = new Date();
        $scope.loadingdiv = true;
        $http.get('https://lookin24.com/sendCommand?imei='+$scope.selectedName.imei+'&youtube=1&curDay=' +$scope.curDay.getTime()+'&off='+$scope.curDay.getTimezoneOffset()).then(function (d) {
            console.log(d.data.data);
            if(d.data.data == undefined) {
                return;
            }
